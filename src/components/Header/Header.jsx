    ...
    <div className="container">
      <div className="flex items-center justify-between relative">
        ...
        <div className="flex items-center px-4">
          <button id="hamburger" type="button" ref={hamburgerRef} onClick={() => setActive(!isActive)} className={`block absolute right-4 lg:hidden ${isActive ? "hamburger-active" : ""}`}>
          ...
          <nav id="navMenu" className={`absolute py-5 rounded-lg max-w-[250px] w-full right-4 top-full shadow-lg bg-white dark:bg-dark dark:shadow-2xl lg:dark:bg-transparent lg:block lg:static lg:bg-transparent lg:max-w-full lg:shadow-none dark:lg:shadow-none ${isActive ? "" : "hidden"}`} ref={menuRef}>
            <ul className="block lg:flex">
              {headerNav?.map((item, i) => (
                <li className="group" key={i}>
                  <a href={item.id} className="text-based text-dark py-2 mx-8 flex dark:text-white group-hover:text-primary">{item.nama}</a>
                </li>
              ))}
              <li className="flex items-center pl-8">
                <div className="flex items-center" onClick={handleThemeSwitch}>
                  ...
                  {darkMode?<BiSolidToggleRight  size={42} className="cursor-pointer text-white"/> : <BiSolidToggleLeft  size={42} className="cursor-pointer"/>}
